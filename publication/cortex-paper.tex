...
\textbf{Availability.} Cortex v1.0.0 is open-source under the Apache-2.0 license. The Rust runtime is published to crates.io (\texttt{cortex-runtime}), the Python client to PyPI (\texttt{cortex-client}), the TypeScript client and MCP server to npm (\texttt{@cortex-ai/client}, \texttt{@cortex/mcp-server}).
